...
if __name__ == "__main__":
    ...
    parser.add_argument('--critiquing_model_name',
                        dest='critiquing_model_name',
                        default="LP1SumToOne",
    ...
    parser.add_argument(
        '--save_path',
        dest='save_path',
        default="beer/critiquing/IncrementalCritiquing/beer_critiquing.csv",
        help='Results saved path. (default: %(default)s)')

    parser.add_argument(
        '--max_iteration_threshold',
        dest='max_iteration_threshold',
        default=10,
        type=check_int_positive,
        help=
        'Maximum iterations allowed for each critiquing session. (default: %(default)s)'
    )

    parser.add_argument(
        '--num_items_sampled',
        dest='num_items_sampled',
        default=5,
        type=check_int_positive,
        help=
        'Number of items sampled for each user in critiquing. (default: %(default)s)'
    )

    parser.add_argument(
        '--num_users_sampled',
        dest='num_users_sampled',
        default=10,
        type=check_int_positive,
    ...
    parser.add_argument(
        '--train_keyphrase',
        dest='train_keyphrase_set',
        default="Rtrain_keyphrase.npz",
        help='Train keyphrase sparse matrix. (default: %(default)s)')

    parser.add_argument(
        '--train_item_keyphrase',
        dest='train_item_keyphrase_set',
        default="Rtrain_item_keyphrase.npz",
